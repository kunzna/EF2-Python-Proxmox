...
# Array to store all IP addresses
all_ips = []
            ...
            if 'net0' in config:
                ...
                for part in net_info.split(','):
                    ...
                    if part.startswith('hwaddr='):
                        mac_addr = part.split('=')[1]
                if ip_addr and ip_addr != '127.0.0.1' and ':' not in ip_addr:  # exclude loopback and IPv6
                    print(f"    IP: {ip_addr} | MAC: {mac_addr if mac_addr else 'Unknown'}")
                    all_ips.append(ip_addr)
            else:
                print("    No network info found in config")
        except Exception as e:
            print(f"    Could not retrieve IP/MAC: {e}")

# Print all collected IPs as comma-separated string
print("\nAll IP addresses:", ",".join(all_ips))
